...
def serializer_context(rf, student, exam):
    ...
    context = {
        'request': request,
        'view': type('FakeView', (object,), {'kwargs': {'student_id': student.id, 'exam_id': exam.id}})()
    }
    return context

def test_successful_submission(db, student, exam, questions, alternatives, serializer_context):
    answers_data = []
    for question in questions:
...
def test_student_already_submitted(db, student, exam, questions, alternatives, serializer_context):
    answers_data = []
    for question in questions:
        correct_alternative = Alternative.objects.get(question=question, is_correct=True)
        answers_data.append({
...
def test_incorrect_number_of_answers(db, student, exam, questions, alternatives, serializer_context):
    ...
    with pytest.raises(ValidationError) as exc_info:
        serializer.is_valid(raise_exception=True)
    assert "The number of answers does not match the number of questions in the exam" in str(exc_info.value)

def test_question_not_in_exam(db, student, exam, questions, alternatives, serializer_context):
    ...
    with pytest.raises(ValidationError) as exc_info:
        serializer.is_valid(raise_exception=True)
    expected_error = f"The question {extra_question.id} does not belong to exam {exam.id}."
    assert expected_error in str(exc_info.value)

def test_duplicate_answers(db, student, exam, questions, alternatives, serializer_context):
    correct_alternative_q1 = Alternative.objects.get(question=questions[0], is_correct=True)
    correct_alternative_q2 = Alternative.objects.filter(question=questions[1], is_correct=False).first()
    answers_data = [
        {
            'question': questions[0].id,
...
def test_alternative_not_belong_to_question(db, student, exam, questions, alternatives, serializer_context):
    alternative_from_other_question = Alternative.objects.filter(question=questions[1]).first()
    correct_alternative_q2 = Alternative.objects.get(question=questions[1], is_correct=True)
    answers_data = [
        {
            'question': questions[0].id,
...
def test_nonexistent_student(db, exam, questions, alternatives, rf):
    ...
    context = {
        'request': request,
        'view': type('FakeView', (object,), {'kwargs': {'student_id': non_existent_student_id, 'exam_id': exam.id}})()
    }
    answers_data = []
    for question in questions:
        correct_alternative = Alternative.objects.get(question=question, is_correct=True)
...
def test_nonexistent_exam(db, student, questions, alternatives, rf):
    ...
    context = {
        'request': request,
        'view': type('FakeView', (object,), {'kwargs': {'student_id': student.id, 'exam_id': non_existent_exam_id}})()
    }
    answers_data = []
    for question in questions:
        correct_alternative = Alternative.objects.get(question=question, is_correct=True)
...
def test_empty_answers_list(db, student, exam, questions, alternatives, serializer_context):
    data = {'answers': []}
    serializer = ExamSubmissionSerializer(data=data, context=serializer_context)
    with pytest.raises(ValidationError) as exc_info:
        serializer.is_valid(raise_exception=True)
...
def test_invalid_data_types(db, student, exam, serializer_context):
    data = {
        'answers': [
            {
                'question': 'invalid_id',
                'selected_alternative': 'invalid_id'
            }
        ]
    }
    serializer = ExamSubmissionSerializer(data=data, context=serializer_context)
...
def test_exam_result_serializer_with_incorrect_answers(db, student, exam, questions, alternatives):
    submission = ExamSubmission.objects.create(student=student, exam=exam)
    for idx, question in enumerate(questions):
        alternatives_q = Alternative.objects.filter(question=question)
        selected_alternative = alternatives_q.filter(is_correct=False).first() if idx == 0 else alternatives_q.get(is_correct=True)
        Answer.objects.create(
            submission=submission,
            question=question,
